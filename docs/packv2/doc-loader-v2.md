...
├── skills/                # pack-wide skills
├── mcp/                   # pack-wide MCP server definitions
...
| Skills | n/a | `skills/` directory (pack-wide) + `agents/<name>/skills/` (per-agent) |
| MCP defs | n/a | `mcp/` directory (pack-wide) + `agents/<name>/mcp/` (per-agent) |
...
cutover: `gc doctor` detects V1 patterns and produces actionable messages;
`gc import migrate` handles the mechanical conversion.
...
1. **`includes` → `[imports]`.** For each `workspace.includes` entry:
   ...
   - Remote URL: synthesize `[imports.<repo-name>]` with `source` and
     `version`. Branch and commit refs are pinned as-is in the lock file;
     only semver tags become version constraints. Other refs become exact
     SHA pins (`version = "sha:<commit>"`).
...
non-semver remote refs) emit warnings the user must review.
